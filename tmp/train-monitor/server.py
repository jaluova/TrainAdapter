...
SNAPSHOT_ENABLED = os.environ.get("TRAIN_MONITOR_SNAPSHOT_ENABLED", "1").strip().lower() in {"1", "true", "yes", "on"}
...
def _sanitize_snapshot_value(value: Any) -> str:
    text = str(value)
...
def _write_snapshot(payload: dict[str, Any]) -> None:
    ...
    latest_step = data.get("latest_step") or {}
    latest_validation = data.get("latest_validation") or {}
    epoch_info = data.get("epoch_info") or {}

    timestamp_ms = payload.get("last_refresh_epoch_ms") or int(time.time() * 1000)
    step = latest_step.get("step")
    epoch = epoch_info.get("current_epoch")
    val_step = latest_validation.get("step")

    name_parts = [time.strftime("%Y%m%d_%H%M%S", time.localtime(timestamp_ms / 1000.0))]
    if step is not None:
        name_parts.append(f"step_{_sanitize_snapshot_value(step)}")
    if epoch is not None:
        name_parts.append(f"epoch_{_sanitize_snapshot_value(epoch)}")
    if val_step is not None:
        name_parts.append(f"val_{_sanitize_snapshot_value(val_step)}")

    snapshot_path = SNAPSHOT_DIR / ("__".join(name_parts) + ".json")
    snapshot_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")

    snapshots = sorted(SNAPSHOT_DIR.glob("*.json"), key=lambda path: path.stat().st_mtime, reverse=True)
    for old_path in snapshots[SNAPSHOT_KEEP:]:
        try:
            old_path.unlink()
...
latest_panel = None
panel_candidates = []
for panel_root_name in ("preview_panels", "qualitative_panels"):
    panel_root = save_dir / panel_root_name
    if not panel_root.exists():
        continue
    for panel_dir in panel_root.iterdir():
        if panel_dir.is_dir():
            panel_candidates.append((panel_dir.stat().st_mtime, panel_root_name, panel_dir))

for _, panel_root_name, panel_dir in sorted(panel_candidates, reverse=True):
    panel_jsons = sorted(panel_dir.glob("sample_*.json"))
    samples = []
    has_nonfinite_score = False
    for sample_path in panel_jsons[:6]:
        ...
        scores = sample_payload.get("prediction_scores") or []
        for score in scores:
            try:
                if not math.isfinite(float(score)):
                    has_nonfinite_score = True
                    break
        ...
        sample_payload["png_name"] = png_path.name if png_path.exists() else None
        sample_payload["panel_root"] = panel_root_name
        if png_path.exists():
            try:
                sample_payload["image_data_url"] = (
                    "data:image/png;base64," +
                    base64.b64encode(png_path.read_bytes()).decode("ascii")
                )
            except Exception:
                sample_payload["image_data_url"] = None
        samples.append(sample_payload)

    if has_nonfinite_score or not samples:
        continue
...
class StatusCache:
    def __init__(self) -> None:
        ...
        self.last_snapshot_val_step: int | None = None
    ...
    def _maybe_snapshot(self, payload: dict[str, Any]) -> None:
        data = payload.get("data") or {}
        latest_step = data.get("latest_step") or {}
        latest_validation = data.get("latest_validation") or {}

        current_step = latest_step.get("step")
        current_val_step = latest_validation.get("step")

        should_snapshot = False
        if current_step is not None:
            if self.last_snapshot_step is None or current_step >= self.last_snapshot_step + SNAPSHOT_STEP_INTERVAL:
                should_snapshot = True
        if current_val_step is not None and current_val_step != self.last_snapshot_val_step:
            should_snapshot = True
        ...
        if current_val_step is not None:
            self.last_snapshot_val_step = current_val_step
